...
public:
	...
	void OnR(vector<string>& operation_seq, vector<Operation>& maneuver);		//R面顺时针转动
	void On_R(vector<string>& operation_seq, vector<Operation>& maneuver);		//R面逆时针转动
	void OnR2(vector<string>& operation_seq, vector<Operation>& maneuver);		//R面转动180°
	void OnF(vector<string>& operation_seq, vector<Operation>& maneuver);		//F面类似
	void On_F(vector<string>& operation_seq, vector<Operation>& maneuver);		//
	void OnF2(vector<string>& operation_seq, vector<Operation>& maneuver);		//
	void OnRR(vector<string>& operation_seq, vector<Operation>& maneuver);		//魔方沿R面顺时针方向整体转动
	void On_RR(vector<string>& operation_seq, vector<Operation>& maneuver);		//魔方沿R面逆时针方向整体转动
	void OnRR2(vector<string>& operation_seq, vector<Operation>& maneuver);		//魔方沿R面逆时针方向整体转动180°
	...
	void LeftLoose(vector<Operation>& maneuver);				//松开左手
	void LeftTight(vector<Operation>& maneuver);				//夹紧左手
	void LeftReady(vector<Operation>& maneuver);				//左手复位至U-D状态(90/270°状态)
